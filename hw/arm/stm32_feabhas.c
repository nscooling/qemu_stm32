...
static void stm32_feabhas_key_event(void *opaque, int keycode)
{
    Stm32Feabhas *s = (Stm32Feabhas *)opaque;
    bool make;
    int core_keycode;
...
static void stm32_feabhas_init(MachineState *machine)
{
    ...
    qemu_add_kbd_event_handler(stm32_feabhas_key_event, s);
...
static QEMUMachine stm32_feabhas_machine = {
    .name = "stm32-feabhas",
    .desc = "Feabhas STM32F407VG Dev Board",
    .init = stm32_feabhas_init,
};

static void stm32_feabhas_machine_init(void)
{
    qemu_register_machine(&stm32_feabhas_machine);
}

machine_init(stm32_feabhas_machine_init);
